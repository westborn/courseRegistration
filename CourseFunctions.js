...
function print_attendance() {
  ...
  var pdfFile = makePDFfromRange(selectedRange, fileName)
...
function print_courseRegister() {
  ...
  var body = sheetToExport.getRange("B3").getDisplayValue() +
...
function createDraftZoomEmail(file) {
...
function makeCourseDetailForWordPress() {
  var rng = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("CourseDetails").getDataRange().getDisplayValues()
  ...
  for (var i=1;i<=numCourses;i++){ //Loop through each row
...
function courseDetailToSheet(course, outputTo){
  ...
  outputTo.setRichTextValue(rich.build())
  ...
  rich = SpreadsheetApp.newRichTextValue();
  rich.setText(cell)
      .setTextStyle(defaultFontSize)
  outputTo.offset(0,1)
          .setRichTextValue(rich.build())
          .setWrapStrategy(SpreadsheetApp.WrapStrategy.WRAP)
          .setVerticalAlignment("middle")
  
  outputTo.offset(0,0,1,2).setBorder(true, null, null, null, null, null, "black", SpreadsheetApp.BorderStyle.SOLID);
